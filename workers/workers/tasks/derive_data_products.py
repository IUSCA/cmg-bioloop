...
def derive_data_products(celery_task, dataset_id: int, conversion_id: int):
    ...
    # Map common analysis type names to their standard file extensions
    # Used when sending file_type as object to API
    ANALYSIS_TYPE_EXTENSIONS = {
        'FASTQ': '.fastq',
        'BAM': '.bam',
        'BIGWIG': '.bw',
        'VCF': '.vcf',
    ...
    for output_dir in output_directories:
        ...
        # Send as object (new schema) for API to connect/create in analysis_type table
        ...
        if analysis_type_config.get('enabled', False) and analysis_type_config.get('value'):
            # Normalize analysis type name: uppercase with underscores
            analysis_type_name = analysis_type_config['value'].strip().upper().replace(' ', '_')
            
            # Look up standard extension, default to generic extension if not found
            analysis_type_extension = ANALYSIS_TYPE_EXTENSIONS.get(
                analysis_type_name,
                f'.{analysis_type_config["value"].lower()}'  # Fallback: use lowercase value as extension
            )
            
            # Send as object: { "name": "FASTQ", "extension": ".fastq" }
            # API will connect to existing analysis_type or create new one
            product_payload["file_type"] = {
                "name": analysis_type_name,
                "extension": analysis_type_extension
            }
