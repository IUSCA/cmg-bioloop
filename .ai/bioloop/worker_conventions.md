...
def await_stability(
    ...
    recency_threshold: int | None = None,
    **kwargs: Any,
) -> tuple[int]:
    ...
...
3. **Worker tasks receive entity_id:**
   ...
   # workers/workers/tasks/process_step_two.py
   def process_step_two(celery_task, entity_id, **kwargs):
       # Same entity_id passed through from step 1
       logger.info(f'Continuing with entity {entity_id}')
...
from workers.api import get_dataset, update_dataset, get_session
