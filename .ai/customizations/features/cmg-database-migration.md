...
- **Change:** Session workflow API endpoint added (`api/src/routes/sessions.js`)
  ...
  - Validates user is session owner (logs warning if unauthorized)
  - Follows standard workflow creation pattern: Rhythm generates workflow ID
  - Creates `session_workflow` record using Rhythm-generated workflow_id
  - Returns workflow response from Rhythm
  - Pattern matches dataset workflow creation (not manual UUID generation)
...
### Bug Fix: Dataset Check Logic for Legacy Sessions

- **Problem:** "View in Genome Browser" showed "No datasets associated with this session" for un-hydrated legacy sessions
  - Root cause: Dataset check uses `session → session_tracks → track → dataset` chain
  - Un-hydrated legacy sessions have NO `session_tracks` records yet (created during hydration)
  - Dataset API returns 0 datasets, UI exits early with warning toast
  - Hydration modal never shows
  
- **Fix:** Reordered logic in `ui/src/pages/sessions/[id].vue`
  - For legacy sessions (`cmg_id` exists), check hydration status FIRST
  - Only check datasets AFTER confirming session is hydrated
  - If not hydrated, show hydration modal immediately (skip dataset check)
  - Updated both `handleViewInBrowser()` and `handleStagingRequested()` functions
  
- **Rationale:** 
  - Bigbang migration creates session records but NOT `session_tracks` (per line 607-613)
  - Only sessions with tracks in CMG are migrated (per `sync_sessions.js` line 46)
  - Hydration creates `session_tracks` records by mapping CMG tracks to Bioloop
  - Cannot check datasets through `session_tracks` until after hydration completes

### Workflow Creation Pattern Fix

- **Problem:** Session workflow endpoint was manually generating workflow IDs using `uuid`
  - Incorrect pattern: Generated UUID, created database record, then called Rhythm API
  - Rhythm API generates its own workflow IDs - manual IDs were incorrect
  - Cleanup logic attempted to delete non-existent database records on failure
  
- **Fix:** Updated to follow standard dataset workflow pattern
  - Call `wfService.create()` first - **Rhythm generates the workflow ID**
  - Use returned `workflow_id` to create database association
  - No cleanup needed in catch block (association doesn't exist yet if creation fails)
  - Added authorization logging for debugging 403 errors
  
- **Documentation:** Added comprehensive workflow creation pattern to `.ai/bioloop/api_conventions.md`
  - Pattern overview with correct flow
  - Real examples from datasets and sessions
  - Common mistakes to avoid
  - Error handling best practices
  
- **Key Principle:** Rhythm is the source of truth for workflow IDs - never generate manually

### Access Control Fix for Session Workflows

- **Problem:** Session workflow endpoint manually checked ownership
  - Code: `if (session.user_id !== req.user.id) return 403`
  - Issue: Blocked operators from hydrating sessions (only checked ownership)
  - Anti-pattern: Duplicates authorization logic that middleware already handles
  
- **Fix:** Removed manual ownership check, rely on `isPermittedTo('update')` middleware
  - Middleware grants access to: session owner, admin role, operator role
  - Consistent with all other Bioloop endpoints
  - Single source of truth for authorization
  
- **Documentation:** Added "Access Control Pattern" section to `.ai/bioloop/api_conventions.md`
  - How `isPermittedTo` middleware works
  - When to use manual checks (rare cases only)
  - Common mistakes to avoid
  - Trust the middleware - don't duplicate authorization logic

### Session Dataset Association via Metadata (2026-01-27)

- **Problem:** Un-hydrated legacy sessions have no `session_tracks`, so couldn't find associated datasets
  - Previous approach: check datasets through `session_tracks` chain
  - Issue: Un-hydrated sessions appear to have no datasets
  - Result: Couldn't determine which datasets need staging before hydration

- **Solution:** Store CMG dataproduct IDs in session metadata during bigbang
  - Bigbang now extracts unique CMG dataproduct IDs from session.tracks
  - Stores in `genome_browser_session.metadata.datasets` array
  - Format: `["cmg_dataproduct_id_1", "cmg_dataproduct_id_2", ...]`
  
- **Change:** Updated bigbang `sync_sessions.js`
  - Extract unique `dataproduct` IDs from `cmgSession.tracks` array
  - Store in `metadata.datasets` field on session creation
  - Logs count of associated dataproducts for debugging
  
- **Change:** Enhanced `GET /sessions/:id/datasets` API endpoint
  - For legacy sessions (`cmg_id` exists with `metadata.datasets`):
    - Maps CMG dataproduct IDs to Bioloop dataset IDs
    - Gets migration status for each legacy dataset
    - Returns datasets with `migration_status` attached
  - For non-legacy or hydrated sessions:
    - Uses existing `session_tracks` approach
  - Consistent response format for both paths
  
- **Change:** Updated UI `handleViewInBrowser()` logic in `/sessions/[id].vue`
  - New flow order:
    1. Get datasets (API automatically uses metadata.datasets for legacy sessions)
    2. Check if datasets need staging/migration
    3. Show staging modal if needed (with correct workflow per dataset)
    4. For legacy sessions: check if all datasets are migrated
    5. For legacy sessions: check if session is hydrated
    6. Show hydration modal if needed
    7. Show browser selection if everything ready
  
- **Change:** Updated `UnstagedDatasetsModal.vue` component
  - Now accepts `datasets` prop (pre-fetched by parent)
  - Each dataset has `needs_migration` flag
  - Triggers `stage` workflow for regular datasets
  - Triggers `stage_migrated` workflow for legacy datasets needing migration
  - Uses same logic as "Browse Files" button on `/datasets/:id` view
  
- **Decision:** Hydration modal only shows when:
  - Session is legacy (has `cmg_id`) AND
  - All legacy datasets have been migrated (checked via `migration_status`) AND
  - Session is not hydrated (checked via `/legacy/migrations/sessions/:id`)
  
- **Rationale:**
  - Cannot hydrate session until tracks exist (requires migrated datasets)
  - Cannot view session until datasets are staged
  - Clear sequential flow: stage → migrate → hydrate → view
  - User sees appropriate modal for current blocker
