 ...
 */
async function acquireLock(pollerName, lockTtlMs = DEFAULT_LOCK_TTL_MS) {
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      logger.debug(`[${pollerName}] Lock acquired by ${INSTANCE_ID}`);
      return updated;
    });
  } catch (error) {
 ...
 */
async function getCursor(pollerName) {
  const prisma = getPrismaClient();
 ...
 */
async function initializeCursor(pollerName, initialUpdatedAt = null, initialObjectId = null) {
  const prisma = getPrismaClient();
