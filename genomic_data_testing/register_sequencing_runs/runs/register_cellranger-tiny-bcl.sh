#!/bin/bash
# =============================================================================
# Cell Ranger Tiny-BCL Sequencing Run Registration Script
# =============================================================================
#
# Purpose:
#   Downloads the Cell Ranger tiny-bcl demo run from 10x Genomics and registers
#   it as RAW_DATA for cellranger mkfastq conversion testing.
#
# Usage:
#   ./register_cellranger-tiny-bcl.sh [OPTIONS]
#
# Options:
...
#
# Dataset Information:
#   - Dataset: cellranger-tiny-bcl-1.2.0.tar.gz
#   - Source: 10x Genomics Cell Ranger demo data
#   - Size: ~37 MB
#   - Type: Illumina BCL run folder (dual-indexed)
#   - Purpose: cellranger mkfastq conversion testing
#
# Target Pipelines:
#   - cellranger (all versions: v4.0.0, v6.1.2, v8.0.1, etc.)
#   - cellranger mkfastq subcommand
#
# Note:
...
# Dataset definition
FILENAME="cellranger-tiny-bcl-1.2.0.tar.gz"
URL="https://cf.10xgenomics.com/supp/cell-exp/cellranger-tiny-bcl-1.2.0.tar.gz"
DIR_NAME="cellranger-tiny-bcl"
RUN_NAME="cellranger-tiny-bcl"
PIPELINE="cellranger"
...
create_conversion_doc() {
    ...
    cat > "$doc_file" << 'EOF'
# Cell Ranger Tiny-BCL Run - cellranger mkfastq Conversion Testing

## Dataset Overview

**Source:** https://cf.10xgenomics.com/supp/cell-exp/cellranger-tiny-bcl-1.2.0.tar.gz  
**File:** `cellranger-tiny-bcl-1.2.0.tar.gz`  
**Size:** ~37 MB

### What This Run Is

- A real Illumina BCL run folder designed for Cell Ranger testing
- Dual-indexed sample (SI-TT-D9 index)
...
This run is ideal for validating the Cell Ranger mkfastq pipeline:

```
BCL -> cellranger mkfastq -> FASTQ files
```

**Important:** This dataset is ONLY for testing mkfastq. It cannot be used with `cellranger count`.

---

## Target Pipelines

This dataset can be used with any of these cellranger versions:

| Pipeline Name | Executable Path |
|--------------|-----------------|
| cellranger | /N/project/CMG-SCA/bin/cellranger/cellranger |
| cellranger-v8.0.1 | /N/project/CMG-SCA/bin/cellranger-8.0.1/cellranger |
| cellranger-v6.1.2 | /N/project/CMG-SCA/bin/cellranger-6.1.2/cellranger |
| cellranger-v4.0.0 | /N/project/CMG-SCA/bin/cellranger-4.0.0/cellranger |

---

## Recommended cellranger mkfastq Configuration

### Simple CSV Sample Sheet

Download from: https://cf.10xgenomics.com/supp/cell-exp/cellranger-tiny-bcl-simple-1.2.0.csv
...
Download from: https://cf.10xgenomics.com/supp/cell-exp/cellranger-tiny-bcl-samplesheet-1.2.0.csv

---

## Correct cellranger mkfastq Command
...
cellranger mkfastq \
  --id=tiny-bcl-output \
  --run=/path/to/cellranger-tiny-bcl \
...
### Pipeline Selection
- Select: `cellranger`, `cellranger-v8.0.1`, `cellranger-v6.1.2`, or `cellranger-v4.0.0`
...
└── _* (other Cell Ranger output files)
```
...
- Valid FASTQ format - Proper structure and encoding
- Contain dual-index reads (I1 and I2)
- NOT suitable for downstream `cellranger count`
...
After running cellranger mkfastq, verify:

- [ ] Exit code is 0
- [ ] `outs/fastq_path/` directory exists
- [ ] Sample directory contains 4 FASTQ files (R1, R2, I1, I2)
...
**Solution:** Ensure using SI-TT-D9 (dual-index) format in sample sheet

**Issue:** "bcl2fastq not found"  
**Solution:** Ensure bcl2fastq is installed and in PATH (cellranger wraps bcl2fastq)
...
- This is a dual-indexed dataset - use `--filter-dual-index` if needed
- The sample index SI-TT-D9 is from the 10x Dual Index Kit TT Set A
...
Download: https://cf.10xgenomics.com/supp/cell-exp/cellranger-tiny-bcl-simple-1.2.0.csv

### IEM Format

Download: https://cf.10xgenomics.com/supp/cell-exp/cellranger-tiny-bcl-samplesheet-1.2.0.csv
...
echo "================================"
echo "Cell Ranger Tiny-BCL Run Registration"
echo "================================"
